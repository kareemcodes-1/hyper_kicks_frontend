...
const CollectionPage = () => {
  ...
  useEffect(() => {
    (async function () {
      const res = await fetch(
        `${import.meta.env.VITE_SERVER_URL}/api/collections/collection/${id}`
      );
    ...
    (async function () {
      try {
        ...
        const res = await fetch(
          ...
          }/api/collections/products/collection/${id}`,
          {
            method: "GET",
            headers: {
  ...
  return (
    <Layout>
      ...
      <div className="lg:mt-[4rem] mt-[5rem] px-[1.5rem] mb-[1rem]">
        <div className="flex lg:items-center items-start lg:flex-row flex-col justify-between w-full">
          <h1 className="lg:text-[8rem] text-black text-[4rem]">
          ...
          <div className="flex items-center gap-[.5rem] relative">
            <button
              type="button"
              className="border border-black rounded-[10rem] py-[.8rem] px-[1rem] flex items-center gap-[.5rem]"
        ...
        <div className="lg:grid flex items-start flex-col grid-cols-3 gap-[1rem] lg:mt-0 mt-[2rem]">
          {loading ? (
            ...
            products
              .filter(
                (product) =>
                  product.collectionId.name.toLowerCase() ===
